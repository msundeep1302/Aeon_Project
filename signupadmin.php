...
require 'db.php';
  ...
  if(@$_POST["submit"]=='Sign Up')
  {
      ...
      $check=mysql_query("select * from sign_up_admin where username='$email'");
      $count=mysql_num_rows($check);
      if($count<=0)
      {
         $hash=md5($password); 
        $qry=mysql_query("insert into sign_up_admin(firstname,lastname,username,password) values('$firstname','$lastname','$email','$hash')"); 
          header('location:loginadmin.php?msg=adminsuc');  
       }
      else
      {
        header('location:signupadmin.php?msg=adminfail');  
      }
   }  
          ...
          <div class="col-sm-5">
            <form method="post" action="">
                <div class="form-group register">
                    <h2 id="sub1">SIGN UP AS ADMIN</h2>
                     <?php 
                       if(@$_GET["msg"]=='adminfail'){
